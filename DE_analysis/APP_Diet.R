...
run_deseq_APP_Diet <- function(cnts, samples,  count_filter, condition) {
  ...
  dds <- DESeqDataSetFromMatrix(countData=cnts2, colData=samples2, design= ~ Diet + APP) 
...
generate_heatmap <- function(dds, results, title){
  ...
  gene_col_2 <- data.frame(Clusters= ifelse(test = gene_col_2 ==1, yes = "Cluster 1", no= "Cluster 2"))
