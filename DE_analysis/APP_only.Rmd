...
#Source your main
source('APP_only.R') # load all our wonderful functions
...
message("Cortex: Running DESeq2 for APP (APP vs WT) only - No Covariates:padj < 0.05 and 0.10 ")
condition = "WT"
Cortex_deseq_results <- run_deseq_APP_only(Cortex_counts_subset_df, Cortex_meta_subset, 10, condition)
...
write.csv(Cortex_results,'Cortex_deseq_results_APP_only.csv')
message("Cortex DESeq2 result is saved in file: 'Cortex_deseq_results_APP_only.csv' ")

# write out matrix of normalized counts
write.csv(counts(Cortex_dds,normalized=TRUE),'Cortex_deseq_norm_counts_APP_only.csv')
message("Cortex DESeq2 normalized counts is saved in file: 'Cortex_deseq_norm_counts_APP_only.csv' ")
...
write.csv(Cortex_deseq_res_with_genename, 'Cortex_deseq_results_with_genename_APP_only.csv')
message("Cortex DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Cortex_deseq_results_with_genename_APP_only.csv' ")
...
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_APP_only_padj_05_filtered.csv')
message("Cortex DESeq2 result after padj (0.05) filtering is saved in file: 'Cortex_deseq_results_APP_only_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Cortex:APP (APP vs WT): ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.05 filtering for Cortex:APP(APP vs WT): ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:APP (APP vs WT):padj < 0.05"
...
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:APP (APP vs WT):padj < 0.05"
...
write.csv(Cortex_deseq_padj_filtered_05, 'Cortex_deseq_results_APP_only_padj_05_filtered_genename.csv')
...
Cortex_deseq_results_padj_filtered <- filter(Cortex_deseq_res, padj < padj_threshold)
print(nrow(Cortex_deseq_results_padj_filtered))
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_APP_only_padj_1_filtered.csv')
message("Cortex DESeq2 result after padj (0.1) filtering is saved in file: 'Cortex_deseq_results_APP_only_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Cortex:APP (APP vs WT): ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.1 filtering for Cortex:APP(APP vs WT) : ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:APP (APP vs WT):padj < 0.1"
Cortex_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_hist_title1)
Cortex_deseq_fc_hist_plot1

#Scatter plots of fold change vs nominal p-value.
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:APP (APP vs WT):padj < 0.1"
Cortex_deseq_scat_plot1 <- deseq_DE_scat_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_scat_title1)
Cortex_deseq_scat_plot1
...
write.csv(Cortex_deseq_padj_filtered_1, 'Cortex_deseq_results_APP_only_padj_1_filtered_genename.csv')
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Cortex:APP (APP vs WT)"
...
```{r Part7}
 # generate heatmap
 Cortex_title <- "Heatmap:Cortex:APP (APP vs WT)"
...
```{r Part8}
 ...
 Cortex_biplot_title = "PCA Plot: PC1 vs PC2:Cortex:APP (APP vs WT)"
 make_biplot(Cortex_metadata_filename, Cortex_pca_results, Cortex_biplot_title)
 
```

9.  Hippocampus: APP (APP vs WT) only - No Covariates (~APP)

```{r Part9}
message("Hippocampus: Running DESeq2 for APP (APP vs WT):padj < 0.05 and 0.10 ")
condition = "Control"
Hippocampus_deseq_results <- run_deseq_APP_only(Hippocampus_counts_subset_df, Hippocampus_meta_subset, 10, condition)
...
Hippocampus_dds
# write out DE results
write.csv(Hippocampus_results,'Hippocampus_deseq_results_APP_only.csv')
message("Hippocampus DESeq2 result is saved in file: 'Hippocampus_deseq_results_APP_only.csv' ")

# write out matrix of normalized counts
write.csv(counts(Hippocampus_dds,normalized=TRUE),'Hippocampus_deseq_norm_counts_APP_only.csv')
message("Hippocampus DESeq2 normalized counts is saved in file: 'Hippocampus_deseq_norm_counts_APP_only.csv' ")
...
write.csv(Hippocampus_deseq_res_with_genename, 'Hippocampus_deseq_results_with_genename_APP_only.csv')
message("Hippocampus DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Hippocampus_deseq_results_with_genename_APP_only.csv' ")
...
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_APP_only_padj_05_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.05) filtering is saved in file: 'Hippocampus_deseq_results_APP_only_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Hippocampus:APP (APP vs WT): ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.05 filtering for Hippocampus:APP (APP vs WT): ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Hippocampus:APP (APP vs WT):padj < 0.05"
...
#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:APP (APP vs WT):padj < 0.05"
...
write.csv(Hippocampus_deseq_padj_filtered_05, 'Hippocampus_deseq_results_APP_only_padj_05_filtered_genename.csv')

############################## padj_threshold = 0.10 ##############################
# filter rows with padj < padj_threshold
padj_threshold <- 0.10
Hippocampus_deseq_results_padj_filtered <- filter(Hippocampus_deseq_res, padj < padj_threshold)
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_APP_only_padj_1_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.1) filtering is saved in file: 'Hippocampus_deseq_results_APP_only_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Hippocampus:APP (APP vs WT): ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.1 filtering for Hippocampus:APP(APP vs WT) only: ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:APP (APP vs WT):padj < 0.1"
Hippocampus_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_hist_title1)
Hippocampus_deseq_fc_hist_plot1

#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:APP (APP vs WT):padj < 0.1"
Hippocampus_deseq_scat_plot1 <- deseq_DE_scat_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_scat_title1)
Hippocampus_deseq_scat_plot1
...
write.csv(Hippocampus_deseq_padj_filtered_1, 'Hippocampus_deseq_results_APP_only_padj_1_filtered_genename.csv')
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Hippocampus:APP (APP vs WT)"
...
```{r Part11}
 # generate heatmap
 Hippocampus_title <- "Heatmap:Hippocampus:APP (APP vs WT)"
...
```{r Part12}
 ...
 Hippocampus_biplot_title = "PCA Plot: PC1 vs PC2:Hippocampus:APP (APP vs WT)"
...
write.csv(Cortex_fgsea_results_mat, 'Cortex_NES_ordered_fgsea_results_APP_only_FC0.csv')
message("Cortex NES ordered fgsea results saved in file: 'Cortex_NES_ordered_fgsea_results_APP_only_FC0.csv' ")
...
Cortex_fgsea_title = "Cortex:APP (APP vs WT)"
...
write.csv(Hippocampus_fgsea_results_mat, 'Hippocampus_NES_ordered_fgsea_results_APP_only_FC0.csv')
message("Hippocampus NES ordered fgsea results saved in file: 'Hippocampus_NES_ordered_fgsea_results_APP_only_FC0.csv' ")

#Plot the top ten pathways with both positive and negative NES (20 total)
#Color the pathways by the sign of their NES (positive or negative)
#top_pathways(fgsea_results, 10)
Hippocampus_fgsea_title = "Hippocampus:APP (APP vs WT)"
