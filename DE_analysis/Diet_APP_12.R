...
run_deseq_Diet_APP_Age <- function(cnts, samples,  count_filter, condition) {
  ...
  dds <- DESeqDataSetFromMatrix(countData=cnts2, colData=samples2, design= ~ APP + Age + Diet) 
  keep <- rowSums(counts(dds)) >= count_filter
  dds <- dds[keep,]
  dds$group <- factor(paste0(dds$APP, dds$Age, dds$Diet))
  design(dds) <- ~ group
  dds <- DESeq(dds)
  ...
  results <- results(dds)
  print(condition)
  results <- results(dds, contrast=condition)
  #results <- results(dds)
  #view(results)
  summary(results)
...
generate_heatmap <- function(dds, results, title){
  ...
  cols <- colData(dds)[,c("Diet", "Age", "APP")]
