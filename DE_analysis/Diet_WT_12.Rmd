...
#Source your main
source('Diet_WT_12.R') # load all our wonderful functions
...
5.  Cortex: APP=WT, Age=12, Diet=Control vs Supplemented (~APP + Age + Diet)
            WT12Control vs WT12Supplemented
   
```{r Part5}

message("Cortex: Running DESeq2 for Diet(Control vs Supplemented) with APP=WT, Age=12:padj < 0.05 and 0.10 ")
message("Cortex: WT12Control vs WT12Supplemented")
condition = c("group", "WT12Control", "WT12Supplemented")
...
write.csv(Cortex_results,'Cortex_deseq_results_Diet_WT_12.csv')
message("Cortex DESeq2 result is saved in file: 'Cortex_deseq_results_Diet_WT_12.csv' ")

# write out matrix of normalized counts
write.csv(counts(Cortex_dds,normalized=TRUE),'Cortex_deseq_norm_counts_Diet_WT_12.csv')
message("Cortex DESeq2 normalized counts is saved in file: 'Cortex_deseq_norm_counts_Diet_WT_12.csv' ")
...
write.csv(Cortex_deseq_res_with_genename, 'Cortex_deseq_results_with_genename_Diet_WT_12.csv')
message("Cortex DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Cortex_deseq_results_with_genename_Diet_WT_12.csv' ")
...
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_Diet_WT_12_padj_05_filtered.csv')
message("Cortex DESeq2 result after padj (0.05) filtering is saved in file: 'Cortex_deseq_results_Diet_WT_12_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Cortex:WT12Control vs WT12Supplemented: ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.05 filtering for Cortex:WT12Control vs WT12Supplemented: ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:WT12Control vs WT12Supplemented:padj < 0.05"
...
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:WT12Control vs WT12Supplemented:padj < 0.05"
...
Cortex_deseq_results_padj_filtered <- filter(Cortex_deseq_res, padj < padj_threshold)
print(nrow(Cortex_deseq_results_padj_filtered))
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_Diet_WT_12_padj_1_filtered.csv')
message("Cortex DESeq2 result after padj (0.1) filtering is saved in file: 'Cortex_deseq_results_Diet_WT_12_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Cortex:WT12Control vs WT12Supplemented: ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.1 filtering for Cortex:WT12Control vs WT12Supplemented: ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:WT12Control vs WT12Supplemented:padj < 0.1"
Cortex_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_hist_title1)
Cortex_deseq_fc_hist_plot1

#Scatter plots of fold change vs nominal p-value.
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:WT12Control vs WT12Supplemented:padj < 0.1"
Cortex_deseq_scat_plot1 <- deseq_DE_scat_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_scat_title1)
Cortex_deseq_scat_plot1
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Cortex:WT12Control vs WT12Supplemented"
...
```{r Part 7}
 # generate heatmap
 Cortex_title <- "Heatmap:Cortex:WT12Control vs WT12Supplemented"
...
8.  Hippocampus: APP=WT, Age=12, Diet=Control vs Supplemented (~APP + Age + Diet)
                 WT12Control vs WT12Supplemented

```{r Part8}
message("Hippocampus: Running DESeq2 for Diet(Control vs Supplemented) with APP=WT, Age=12:padj < 0.05 and 0.10 ")
message("Hippocampus: WT12Control vs WT12Supplemented")
condition = c("group", "WT12Control", "WT12Supplemented")
...
Hippocampus_dds
# write out DE results
write.csv(Hippocampus_results,'Hippocampus_deseq_results_Diet_WT_12.csv')
message("Hippocampus DESeq2 result is saved in file: 'Hippocampus_deseq_results_Diet_WT_12.csv' ")

# write out matrix of normalized counts
write.csv(counts(Hippocampus_dds,normalized=TRUE),'Hippocampus_deseq_norm_counts_Diet_WT_12.csv')
message("Hippocampus DESeq2 normalized counts is saved in file: 'Hippocampus_deseq_norm_counts_Diet_WT_12.csv' ")
...
write.csv(Hippocampus_deseq_res_with_genename, 'Hippocampus_deseq_results_with_genename_Diet_WT_12.csv')
message("Hippocampus DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Hippocampus_deseq_results_with_genename_Diet_WT_12.csv' ")
...
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_Diet_WT_12_padj_05_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.05) filtering is saved in file: 'Hippocampus_deseq_results_Diet_WT_12_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Hippocampus:WT12Control vs WT12Supplemented: ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.05 filtering for Hippocampus:WT12Control vs WT12Supplemented: ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Hippocampus:WT12Control vs WT12Supplemented:padj < 0.05"
...
#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:WT12Control vs WT12Supplemented:padj < 0.05"
...
padj_threshold <- 0.10
Hippocampus_deseq_results_padj_filtered <- filter(Hippocampus_deseq_res, padj < padj_threshold)
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_Diet_WT_12_padj_1_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.1) filtering is saved in file: 'Hippocampus_deseq_results_Diet_WT_12_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Hippocampus:WT12Control vs WT12Supplemented: ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.1 filtering for Hippocampus:WT12Control vs WT12Supplemented: ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Hippocampus:WT12Control vs WT12Supplemented:padj < 0.10"
Hippocampus_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_hist_title1)
Hippocampus_deseq_fc_hist_plot1

#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:WT12Control vs WT12Supplemented:padj < 0.10"
Hippocampus_deseq_scat_plot1 <- deseq_DE_scat_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_scat_title1)
Hippocampus_deseq_scat_plot1
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Hippocampus:WT12Control vs WT12Supplemented"
...
```{r Part10}
 # generate heatmap
 Hippocampus_title <- "Heatmap:Hippocampus:WT12Control vs WT12Supplemented"
...
write.csv(Cortex_fgsea_results_mat, 'Cortex_NES_ordered_fgsea_results_Diet_WT_12_FC0.csv')
message("Cortex NES ordered fgsea results saved in file: 'Cortex_NES_ordered_fgsea_results_Diet_WT_12_FC0.csv' ")
...
Cortex_fgsea_title = "Cortex:WT12Control vs WT12Supplemented"
...
write.csv(Hippocampus_fgsea_results_mat, 'Hippocampus_NES_ordered_fgsea_results_Diet_WT_12_FC0.csv')
message("Hippocampus NES ordered fgsea results saved in file: 'Hippocampus_NES_ordered_fgsea_results_Diet_WT_12_FC0.csv' ")

#Plot the top ten pathways with both positive and negative NES (20 total)
#Color the pathways by the sign of their NES (positive or negative)
#top_pathways(fgsea_results, 10)
Hippocampus_fgsea_title = "Hippocampus:WT12Control vs WT12Supplemented"
