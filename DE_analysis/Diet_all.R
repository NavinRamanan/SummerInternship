...
run_deseq_Diet_all <- function(cnts, samples,  count_filter, condition) {
  ...
  dds <- DESeqDataSetFromMatrix(countData=cnts2, colData=samples2, design= ~ Sex + Age + APP + Diet) 
...
generate_heatmap <- function(dds, results, title){
  ...
  cols <- colData(dds)[,c("Diet", "APP", "Age", "Sex")]
