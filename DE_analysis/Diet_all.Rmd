...
#Source your main
source('Diet_all.R') # load all our wonderful functions
...
nrow(Cortex_counts_subset)
# Keep rows with sum of sample counts greater than 10
Cortex_counts_subset_df <- Cortex_counts_subset[(rowSums(Cortex_counts_subset) > 10),]
nrow(Cortex_counts_subset_df)
#view(Cortex_counts_subset_df)
#Cortex_counts_subset_df <- Cortex_counts_subset
```

5.  Cortex: Control vs Supplemented Diet – controlling for all covariates (~Sex + Age + APP + Diet)
   
```{r Part5}

message("Cortex: Running DESeq2 for Diet(Control vs Supplemented) while controlling for Sex, Age and APP:padj < 0.05 and 0.10 ")
message("Cortex: Sex(Constant) + Age(Constant) + APP(Constant) + Diet(Varied i.e., Control vs Supplemented):padj < 0.05 and 0.10")
condition = "Control"
Cortex_deseq_results <- run_deseq_Diet_all(Cortex_counts_subset_df, Cortex_meta_subset, 10, condition)
...
write.csv(Cortex_results,'Cortex_deseq_results_Diet_all.csv')
message("Cortex DESeq2 result is saved in file: 'Cortex_deseq_results_Diet_all.csv' ")

# write out matrix of normalized counts
write.csv(counts(Cortex_dds,normalized=TRUE),'Cortex_deseq_norm_counts_Diet_all.csv')
message("Cortex DESeq2 normalized counts is saved in file: 'Cortex_deseq_norm_counts_Diet_all.csv' ")
...
write.csv(Cortex_deseq_res_with_genename, 'Cortex_deseq_results_with_genename_Diet_all.csv')
message("Cortex DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Cortex_deseq_results_with_genename_Diet_all.csv' ")
...
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_Diet_all_padj_05_filtered.csv')
message("Cortex DESeq2 result after padj (0.05) filtering is saved in file: 'Cortex_deseq_results_Diet_all_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Cortex:Control vs Supplemented Diet while controlling Sex, Age and APP: ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.05 filtering for Cortex:Control vs Supplemented Diet while controlling Sex, Age and APP: ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.05"
...
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.05"
...
view(Cortex_deseq_padj_filtered_05)
...
write.csv(Cortex_deseq_padj_filtered_05, 'Cortex_deseq_results_Diet_all_padj_05_filtered_genename.csv')
...
Cortex_deseq_results_padj_filtered <- filter(Cortex_deseq_res, padj < padj_threshold)
print(nrow(Cortex_deseq_results_padj_filtered))
write.csv(Cortex_deseq_results_padj_filtered, 'Cortex_deseq_results_Diet_all_padj_1_filtered.csv')
message("Cortex DESeq2 result after padj (0.1) filtering is saved in file: 'Cortex_deseq_results_Diet_all_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Cortex:Control vs Supplemented Diet while controlling Sex, Age and APP: ", nrow(Cortex_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Cortex_deseq_res) - nrow(Cortex_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold 0.1 filtering for Cortex:Control vs Supplemented Diet while controlling Sex, Age and APP: ", no_of_genes_discarded))
  
#Histograms of fold change values from the significant DE genes. 
Cortex_deseq_hist_title1 <- "Histogram of log2FoldChange:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.1"
Cortex_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_hist_title1)
Cortex_deseq_fc_hist_plot1

#Scatter plots of fold change vs nominal p-value.
Cortex_deseq_scat_title1 <- "Scatter Plot:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.1"
Cortex_deseq_scat_plot1 <- deseq_DE_scat_plot(Cortex_deseq_results_padj_filtered, Cortex_deseq_scat_title1)
Cortex_deseq_scat_plot1
...
write.csv(Cortex_deseq_padj_filtered_1, 'Cortex_deseq_results_Diet_all_padj_1_filtered_genename.csv')
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
```{r Part 7}
 # generate heatmap
 Cortex_title <- "Heatmap:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
```{r Part 8}
 ...
 Cortex_biplot_title = "PCA Plot: PC1 vs PC2:Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
 make_biplot(Cortex_metadata_filename, Cortex_pca_results, Cortex_biplot_title)
 
```


9.  Hippocampus: Control vs Supplemented Diet – controlling for all covariates (~Sex + Age + APP + Diet)

```{r Part9}
message("Hippocampus: Running DESeq2 for Diet(Control vs Supplemented) while controlling for Sex, Age and APP:padj < 0.05 and 0.10 ")
message("Hippocampus: Sex(Constant) + Age(Constant) + APP(Constant) + Diet(Varied i.e., Control vs Supplemented):padj < 0.05 and 0.10")
condition = "Control"
Hippocampus_deseq_results <- run_deseq_Diet_all(Hippocampus_counts_subset_df, Hippocampus_meta_subset, 10, condition)
...
Hippocampus_dds
# write out DE results
write.csv(Hippocampus_results,'Hippocampus_deseq_results_Diet_all.csv')
message("Hippocampus DESeq2 result is saved in file: 'Hippocampus_deseq_results_Diet_all.csv' ")

# write out matrix of normalized counts
write.csv(counts(Hippocampus_dds,normalized=TRUE),'Hippocampus_deseq_norm_counts_Diet_all.csv')
message("Hippocampus DESeq2 normalized counts is saved in file: 'Hippocampus_deseq_norm_counts_Diet_all.csv' ")
...
write.csv(Hippocampus_deseq_res_with_genename, 'Hippocampus_deseq_results_with_genename_Diet_all.csv')
message("Hippocampus DESeq2 result with Gene names (mgi_symbols) is saved in file: 'Hippocampus_deseq_results_with_genename_Diet_all.csv' ")
...
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_Diet_all_padj_05_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.05) filtering is saved in file: 'Hippocampus_deseq_results_Diet_all_padj_05_filtered.csv' ")

# Number of genes significant at padj less than 0.05
message(paste0("Number of DE genes significant at padj < 0.05 for Hippocampus:Control vs Supplemented Diet while controlling Sex, Age and APP: ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.05 filtering for Hippocampus:Control vs Supplemented Diet while controlling Sex, Age and APP: ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.05"
...
#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.05"
...
write.csv(Hippocampus_deseq_padj_filtered_05, 'Hippocampus_deseq_results_Diet_all_padj_05_filtered_genename.csv')

############################## padj_threshold = 0.10 ##############################
# filter rows with padj < padj_threshold
padj_threshold <- 0.10
Hippocampus_deseq_results_padj_filtered <- filter(Hippocampus_deseq_res, padj < padj_threshold)
write.csv(Hippocampus_deseq_results_padj_filtered, 'Hippocampus_deseq_results_Diet_all_padj_1_filtered.csv')
message("Hippocampus DESeq2 result after padj (0.1) filtering is saved in file: 'Hippocampus_deseq_results_Diet_all_padj_1_filtered.csv' ")

# Number of genes significant at padj less than 0.1
message(paste0("Number of DE genes significant at padj < 0.1 for Hippocampus:Control vs Supplemented Diet while controlling Sex, Age and APP: ", nrow(Hippocampus_deseq_results_padj_filtered)))

# Number of DE Genes discarded
no_of_genes_discarded <- nrow(Hippocampus_deseq_res) - nrow(Hippocampus_deseq_results_padj_filtered)
message(paste0("Number of DE genes discarded after padj threshold < 0.1 filtering for Hippocampus:Control vs Supplemented Diet while controlling Sex, Age and APP: ", no_of_genes_discarded))

Hippocampus_deseq_hist_title1 <- "Histogram of log2FoldChange:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.10"
Hippocampus_deseq_fc_hist_plot1 <- deseq_FC_hist_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_hist_title1)
Hippocampus_deseq_fc_hist_plot1

#scatter plot
Hippocampus_deseq_scat_title1 <- "Scatter Plot:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled:padj < 0.10"
Hippocampus_deseq_scat_plot1 <- deseq_DE_scat_plot(Hippocampus_deseq_results_padj_filtered, Hippocampus_deseq_scat_title1)
Hippocampus_deseq_scat_plot1
...
write.csv(Hippocampus_deseq_padj_filtered_1, 'Hippocampus_deseq_results_Diet_all_padj_1_filtered_genename.csv')
...
padj_threshold <- 0.05
FC_threshold <- 0
subtitle <- "DESeq2 Results:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
```{r Part11}
 # generate heatmap
 Hippocampus_title <- "Heatmap:Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
```{r Part12}
 ...
 Hippocampus_biplot_title = "PCA Plot: PC1 vs PC2:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
write.csv(Cortex_fgsea_results_mat, 'Cortex_NES_ordered_fgsea_results_Diet_all_FC0.csv')
message("Cortex NES ordered fgsea results saved in file: 'Cortex_NES_ordered_fgsea_results_Diet_all_FC0.csv' ")
...
Cortex_fgsea_title = "Cortex:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
...
write.csv(Hippocampus_fgsea_results_mat, 'Hippocampus_NES_ordered_fgsea_results_Diet_all_FC0.csv')
message("Hippocampus NES ordered fgsea results saved in file: 'Hippocampus_NES_ordered_fgsea_results_Diet_all_FC0.csv' ")

#Plot the top ten pathways with both positive and negative NES (20 total)
#Color the pathways by the sign of their NES (positive or negative)
#top_pathways(fgsea_results, 10)
Hippocampus_fgsea_title = "Hippocampus:Diet (Control vs Supplemented) - Sex, Age and APP Controlled"
