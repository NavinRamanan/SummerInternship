...
run_deseq_Diet_only <- function(cnts, samples,  count_filter, condition) {
  ...
  dds <- DESeqDataSetFromMatrix(countData=cnts2, colData=samples2, design= ~ Diet) 
...
# Diet(Control vs Supplemented)
make_biplot <- function(metadata, pca_results, title) {
  meta_data <- read.csv(metadata)
  meta_data <- data.frame(meta_data)
  # select only Sample and Diet which is the GSM sample data from meta_data
  meta_df <- meta_data %>% dplyr::select(Diet, Sample)   
  ...
  # Now change the row names in pca_results$x to Diet names from meta data
  # which we will use to color code the PC1 and PC2 data in the plotting
  row.names(pca_results$x) <- meta_df_sample[,"Diet"]
  Diet = c(rownames(pca_results$x))
  s <- summary(pca_results)
  #str(summary(pca_results))
  
  bplot <- 
    pca_results$x %>% as.data.frame %>%
    ggplot(aes(x=PC1,y=PC2, col=Diet)) + 
    geom_point() + 
    #theme_classic() +
    theme_bw() +
    labs(title=title) +
